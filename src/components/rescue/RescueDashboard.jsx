...
import { claimIncident } from '../../services/cloudSync.js';
import { normalizeIncident } from '../../services/incidentBridge.js';
...
export default function RescueDashboard({ refreshTrigger }) {
  ...
  const handleClaim = async (packet) => {
    const targetId = packet.cloudId || packet.id;

    setIncidents((prev) =>
      prev.map((incident) =>
        incident.id === packet.id
          ? { ...incident, status: 'claimed', claimed_by: 'STRIKE_ALPHA', team: 'STRIKE_ALPHA' }
          : incident,
      ),
    );

    try {
      await claimIncident(targetId, 'STRIKE_ALPHA');
    } catch (error) {
      console.error('❌ [Dispatch] Claim failed:', error);
      setIncidents((prev) =>
        prev.map((incident) =>
          incident.id === packet.id
            ? { ...incident, status: packet.status, claimed_by: packet.claimed_by, team: packet.team }
            : incident,
        ),
      );
    }
  ...
  const spawnSimulation = () => {
    ...
    const mock = Array.from({ length: 4 }).map((_, i) => {
      ...
      return normalizeIncident({
        ...
        status: 'active',
        category: 'SIMULATED_SIGNAL',
        category_code: types[i % 4],
  ...
  const filtered = incidents.filter(p => {
    if (activeTab === 'pending') return !p.status || p.status === 'active';
    if (activeTab === 'responded') return p.status === 'claimed';
