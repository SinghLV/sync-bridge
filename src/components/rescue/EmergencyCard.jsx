...
export default function EmergencyCard({ packet, onClaim, showClaim }) {
  const { severity, ts, id, status } = packet;
  const config = SEVERITY_LABELS[severity] || SEVERITY_LABELS.standard;
  const assignedTeam = packet.claimed_by || packet.team;
        ...
        <div style={CardStyles.footer}>
          {status === 'claimed' ? (
            <div style={{ ...CardStyles.claimedIndicator, borderColor: config.color }}>
              ...
              <div className="technical-data" style={CardStyles.teamName}>{assignedTeam || 'STRIKE_TEAM'}</div>
            </div>
          ) : showClaim ? (
